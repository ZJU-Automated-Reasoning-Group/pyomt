...
def opt_with_qsmt(fml: z3.ExprRef, obj: z3.ExprRef, minimize: bool, solver_name: str):
...
def demo_qsmt():
    ...
    fml = z3.Implies(z3.And(z3.UGT(y, 0), z3.ULT(y, 10)), z3.ULT(y - 2 * x, 7))
    print("start solving")
    res = opt_with_qsmt(fml, x, minimize=True, solver_name="z3")
    print(res)
    start = time.time()
